...
HTTP API lives in `app/api/` (thin routes, `Depends`, Pydantic DTOs). Customer Portal and Support Console are Next.js in **`web/`** — do not serve the UI with `app.frontend()`. Live chat SSE + Redis: load `realtime-chat` as well as this skill.
